...
def process_file(file_path: str, base_module_name: str) -> list:
    ...
    relationship_analyzer = ast_collectors.ClassRelationshipInspector(import_aliases, class_metadata_list)
    for index, metadata in enumerate(class_metadata_list):
        metadata.relationships = relationship_analyzer.visit(class_nodes[index])
...
def generate_classes_dicts_from_directory(directory_path: str) -> list:
    ...
    Returns:
        list: A list of dictionaries representing class metadata.
    """
    python_file_paths = file_mgmt.find_files_with_extension(directory_path, ".py")
    ...
    for file_path in python_file_paths:
        combined_class_metadata_list.extend(process_file(file_path, base_module_name))

    # Ensure all relationships are accounted for
    all_class_names = {metadata.name for metadata in combined_class_metadata_list}
    for metadata in combined_class_metadata_list:
        for relationship in metadata.relationships:
            if relationship.type != "association" and relationship.related not in all_class_names:
    ...
    combined_class_metadata_dicts = [metadata.to_dictionary() for metadata in combined_class_metadata_list]
